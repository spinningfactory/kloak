...
fn main() {
    ...
    let sep = "=".repeat(60);
    println!("{}", sep);
    println!("Kloak Rust Demo: ureq + rustls (NOT interceptable)");
    println!("{}", sep);
    println!("Target URL: {}\n", target_url);
    ...
    loop {
        ...
        let result = ureq::get(&target_url)
            .set("X-Secret-Allowed", &key_allowed)
            .set("X-Secret-Blocked", &key_blocked)
            .set("Authorization", &format!("Bearer {}", key_allowed))
            .call();
