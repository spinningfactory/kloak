...
fn main() {
    ...
    let sep = "=".repeat(60);
    println!("{}", sep);
    println!("Kloak Rust Demo: attohttpc + native-tls (OpenSSL, default)");
    println!("{}", sep);
    println!("Target URL: {}\n", target_url);
    ...
    loop {
        ...
        let result = attohttpc::get(&target_url)
            .header("X-Secret-Allowed", &key_allowed)
            .header("X-Secret-Blocked", &key_blocked)
            .header("Authorization", format!("Bearer {}", key_allowed))
            .send();
