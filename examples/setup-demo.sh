...
deploy_kloak() {
    ...
    # Wait for controller to generate certificates (auto-cert mode)
    echo "Waiting for controller to generate certificates..."
    until kubectl get secret kloak-ca -n kloak-system 2>/dev/null; do
        echo -n "."
        sleep 2
    done
    echo ""
