...
int cgroup_connect4(struct bpf_sock_addr *ctx) {
    ...
    bpf_printk("bouncer: redirected %pI4:%d -> 127.0.0.1:%d",
               &dst.ip4, dst_port, ENVOY_PORT);

    return 1; // Allow (with modified destination)
}

// IPv6 connect hook (simplified - redirects to IPv4 localhost)
SEC("cgroup/connect6")
int cgroup_connect6(struct bpf_sock_addr *ctx) {
    ...
    // A full implementation would store the complete IPv6 address
    __u64 cookie = bpf_get_socket_cookie(ctx);
    struct orig_dst dst = {
